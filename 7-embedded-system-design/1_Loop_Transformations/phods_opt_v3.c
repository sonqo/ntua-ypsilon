                ...
                for (i=-S; i<S+1; i+=S) {
                    ...
                    for (k=0; k<Bx; k++) {
                        for (l=0; l<By; l++) {
                            ...
                            if ((Bx*x+vectors_x[x][y]+i+k) < 0 || (Bx*x+vectors_x[x][y]+i+k) > (N-1) || (By*y+vectors_y[x][y]+l) < 0 || (By*y+vectors_y[x][y]+l) > (M-1)) {
                                p2 = 0;
                            } else {
                            ...
                            if (i == 0) {
                                ...
                                if ((Bx*x+vectors_x[x][y]+k) < 0 || (Bx*x+vectors_x[x][y]+k) > (N-1) || (By*y+vectors_y[x][y]+i+l) < 0 || (By*y+vectors_y[x][y]+i+l) > (M-1)) {
                                    q2 = 0;
                                } else {
...
int main(int argc, char* argv[]) {  
    ...
    int **motion_vectors_x = malloc((N/Bx) * sizeof(int*));
    for (i=0; i<(N/Bx); i++) {
        motion_vectors_x[i] = malloc((M/By) * sizeof(int));
    }

    int **motion_vectors_y = malloc((N/Bx) * sizeof(int*));
    for (i=0; i<(N/Bx); i++) {
        motion_vectors_y[i] = malloc((M/By) * sizeof(int));
    }
